...
from src.schemas.smoothing import SmoothedCandlesticksQueryInputSchema, SmoothingInputSchema
...
class SmoothingService:
    ...
    async def extract_smoothed_candlesticks(self, input_schema: SmoothedCandlesticksQueryInputSchema) -> DataFrame:
        smoothed_candlesticks: DataFrame = await self._repository.query_smoothed_candlesticks(input_schema=input_schema)
        return smoothed_candlesticks.drop_duplicates()
